...
- 🆕 **資訊架構整理(2026-08-14)**。**這一整項不在任何 spec 的原始範圍內** —— 對標與規格都只寫功能清單,沒有一項在講操作動線,這是實際使用後才發現的缺口:
  - **工具列**:原本 14 顆平權按鈕已溢出視窗(「提示詞總表」被切掉),改成頂部 4 顆常用 + 左側 icon 工具欄(依流程分四組:素材設定 / 匯入 / 檢查剪接 / 產出)。抽出 `ToolButton` 讓「圖示+文字」與「純圖示+hover 標籤」兩種樣式只寫一份
  - **首頁**:備份 / Prompt 模板 / 模型設定收進「設定」下拉,header 從 5 顆變 3 顆
  - **分鏡編輯器**:關鍵幀與影片兩個重面板改分頁。實測內容高 1080px → 731px,在 1280×800 下不用再滾。影片是非同步任務,所以分頁標籤帶狀態(生成中 / 已完成 / 失敗),否則跑完不會被發現
  - **命名**:資產編輯器標題原本寫死「新增角色」(內容早已泛化成四種資產),改成跟著種類走;專案內的「角色設定」改名「本片角色表」並明講它**只是文字備註**,與「資產庫」的參考圖是不同的東西 —— 兩個概念並排卻沒講清差別,是先前最容易誤解的地方
