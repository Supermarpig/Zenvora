...
- ✅ **M0 影片 pipeline**:`VideoProvider` 抽象 + Veo provider + 任務 store/輪詢 + 下載代理 + 分鏡影片面板 + 節點影片標記。程式與降級 UX 完整;**實際產出影片需你的 Google 金鑰已開通 Veo 權限且有額度**。
- ✅ **M1 人物資產庫**:全域角色庫(`/characters`)+ 多角度設定圖一鍵生成 + 上傳參考圖 + 分鏡選角 + 生圖自動帶入外觀與參考圖。已在瀏覽器驗證(建立/編輯/選角皆正常);`tsc` 與 `eslint` 全綠。
- ⏳ 待辦:M2 時間軸、M3 fal 多引擎、M4 帶貨、M5 換人物、M6 正式化。
- ⚠️ 驗證時你的 Gemini 金鑰回 **429 配額用盡**,故未能跑出實際生成結果;錯誤處理已驗證正常(以 toast 顯示、UI 優雅降級)。額度恢復後即可實測生成。
