...
共 9 個(行號會隨改動漂移,以檔名為準):

- `create-project-dialog.tsx` — `CreateProjectInput` unused
- `prompt-row.tsx` / `storyboard-grid.tsx` / `frame-node.tsx` / `image-generator.tsx` / `timeline-preview-dialog.tsx` — `<img>` 而非 `next/image`
- `frame-editor.tsx` — `form.watch()` 無法被 React Compiler memo
- `image-generator.tsx` — `ImageIcon` unused
- `proxy.ts` — `_request` unused(它是 no-op,見檔內註解)
...
- **備份用 `src/lib/snapshot.ts` + `zip.ts` 的 createZip/readZip**。`readZip` 只支援 store 模式(本專案自己產的備份),遇到 deflate 會明確報錯。素材的 `kind` 欄位決定還原成 data URL 字串或 Blob —— 圖片與影片在 IndexedDB 的儲存形態不同。
- **結束幀(首尾插值)獨立於起始幀**:`endimage-` 前綴自己一組 db 函式,不是給 `saveImage` 加參數 —— 起始幀是「這一鏡的圖」,結束幀是「這一鏡的生成約束」,生命週期不同。`frameSchema.endImageKey` 同樣只當有無的標記。UI 只在 `supportsEndFrame && 有起始幀 && 鏈路非 t2v` 時顯示。
- **引擎能力寫在 `VIDEO_MODELS` 註冊表**(`supportsEndFrame` / `supportedAspects` / `allowedDurations`),UI 依此過濾。Veo 只吃 16:9 與 9:16、秒數只接受 4/6/8 —— 先前 UI 提供 1:1 與連續秒數,選了會被 API 打回來。`snapDuration()` 負責吸附並在 UI 明講。
